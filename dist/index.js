...
const isRgb = util.hasAllCharkeys(['r', 'g', 'b']);
const isHsl = util.hasAllCharkeys(['h', 's', 'l']);
const isHsv = util.hasAllCharkeys(['h', 's', 'v']);

const round2 = util.rounder({
  decimals: 2
});
function colorToCss(obj) {
  obj = util.charkeys(obj);
  let fn;
...
const clip1 = util.clipper({
  min: 0,
  max: 1
});
const clip255 = util.clipper({
  min: 0,
  max: 255
});
const clip360 = util.clipper({
  min: 0,
  max: 360
});
class Color {
  ...
  _setData({
    ...
    if (!util.defined(alpha) || Number.isNaN(alpha)) {
  ...
  static random() {
    ...
    for (let i = 0; i < 3; i++) {
      const component = util.randomInt({
        max: 255
      });
